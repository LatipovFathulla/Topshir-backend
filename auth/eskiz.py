...
class SendSmsApiWithEskiz:
    ...
    def authorization(self):
        ...
        if 'data' in r.json() and 'token' in r.json()['data']:
    ...
    def get_status(self, id):
        ...
        if 'status' in r.json() and r.json()['status'] == 'success':
            if 'message' in r.json() and 'status' in r.json()['message']:
    ...
    def clean_message(self, message):
        ...
        message = message.replace('ц', 'ts').replace('ч', 'ch').replace('ю', 'yu').replace('а',
                                                                                           'a').replace('б',
                                                                                                        'b').replace(
            'қ', "q").replace('ў', "o'").replace('ғ', "g'").replace('ҳ', "h").replace('х', "x").replace('в',
                                                                                                        'v').replace(
            'г', 'g').replace('д', 'd').replace('е', 'e').replace('ё', 'yo').replace('ж', 'j').replace('з',
                                                                                                       'z').replace(
            'и',
            'i').replace(
            'й', 'y').replace('к', 'k').replace('л', 'l').replace('м', 'm').replace('н', 'n').replace('о',
                                                                                                      'o').replace(
            'п', 'p').replace('р', 'r').replace('с', 's').replace('т', 't').replace('у', 'u').replace('ш',
                                                                                                      'sh').replace(
            'щ',
            'sh').replace(
            'ф', 'f').replace('э', 'e').replace('ы', 'i').replace('я', 'ya').replace('ў', "o'").replace('ь',
                                                                                                        "'").replace(
            'ъ', "'").replace('’', "'").replace('“', '"').replace('”', '"').replace(',', ',').replace('.',
                                                                                                      '.').replace(
            ':', ':')
        # filter upper
        message = message.replace('Ц', 'Ts').replace('Ч', 'Ch').replace('Ю', 'Yu').replace('А',
                                                                                           'A').replace('Б',
                                                                                                        'B').replace(
            'Қ', "Q").replace('Ғ', "G'").replace('Ҳ', "H").replace('Х', "X").replace('В', 'V').replace('Г',
                                                                                                       'G').replace(
            'Д',
            'D').replace(
            'Е', 'E').replace('Ё', 'Yo').replace('Ж', 'J').replace('З', 'Z').replace('И', 'I').replace('Й',
                                                                                                       'Y').replace(
            'К',
            'K').replace(
    ...
    def calculation_send_sms(self, message):
        try:
            ...
            if length >= 0 and length <= 160:
                ...
                self.spend = 8
            elif length > 1224 and length <= 1377:
                self.spend = 9
            elif length > 1377 and length <= 1530:
                self.spend = 10
            else:
                self.spend = int(length / 1530)
                if length % 1530 != 0:
                    self.spend += 1
        except:
            return FAILED
